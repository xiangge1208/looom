...
这台 Doris 上有**三个**库，其中两个不属于本项目：

| 库名 | 状态 | 处置 |
|---|---|---|
| `reroll_analysis` | 已有数据 | **绝对不要读写或修改**（goal.md 明令） |
| **`loom`** | **已有 ~28 张表、80 万+ 行真实亚马逊业务数据** | **绝对不要碰**。见下方说明 |
| `looom` | 本项目库（三个 o） | 我们的工作区 |

### 关于 `loom` 库（重要）

侦察进入 P0 时发现这台 Doris 上已存在一个 `loom` 库（比本项目名少一个 o），
...
> ⚠️ **命名极易混淆**：`loom` vs `looom` 只差一个字母。
> 写 SQL 时务必确认库名，`scripts/setup-doris.sh` 里已加护栏
> （`DB_NAME` 若为 `reroll_analysis` / `loom` / `mysql` 等会直接拒绝执行）。
...
原因：43 张业务表字段高度同构（渠道长表的 5 个指标列在多表复用），
