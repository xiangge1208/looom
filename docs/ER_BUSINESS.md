...
> ⚠️ **本文档已过期，仅保留作历史参考。**
> 撰写时是 schema-02（43 张业务表）时代的快照。此后经过：
> `schema-03` 补 3 张、`schema-04` 把 16 张关键词表的主键从 `keyword_id`
> 改成 `(keyword, country)` 文本键、`schema-05` 两张大表分区迁移、
> `schema-06/07` M13 又增删若干。
>
> **当前实况请以 [DORIS_SCHEMA_DESIGN.md §14](DORIS_SCHEMA_DESIGN.md) 为准**
> （65 张物理表 = 61 张逻辑表 + 4 张迁移残留）。
> 尤其注意下面这张图画的是 **keyword_id 作主键**，那套设计**已被 schema-04 推翻**
> ——实际是文本键，`keyword_id` 只是可空的对账列。照本图写 JOIN 会出错。
>
> 保留原因：图中「谁连谁」的**关系拓扑**（变体组、ASIN×关键词、广告层级、
> 推荐专栏钻取）至今仍成立，只是键列名要换。看结构可以，抄键列不行。
...
表数量（**撰写时口径，已过期**）：`dim_` 8 · `fact_` 18 · `rel_` 6 · `dict_` 11 = **43 张**。
